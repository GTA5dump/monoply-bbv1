...
namespace monopoly
{
		...
		if (toggle && *g_pointers->m_is_session_started)
		{
			...
			if (ImGui::Begin(xorstr_("##sessionInfo"), nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoBackground))
			{
				if (!isUpdating)
				{
					isUpdating = true;
					std::thread([&]() {
						while (toggle && *g_pointers->m_is_session_started)
						{
							connectedPlayers = gta_util::get_connected_players();
				...
				ImGui::Text(xorstr_("Players: %d"), connectedPlayers);
				ImGui::Text(xorstr_("Free slots: %d"), 32 - connectedPlayers);
			}
		}
	}
	...
	void features::changemodel(const char* model)
	{
		...
		if (STREAMING::IS_MODEL_IN_CDIMAGE(model1) && STREAMING::IS_MODEL_VALID(model1))
		{
			STREAMING::REQUEST_MODEL(model1);
			while (!STREAMING::HAS_MODEL_LOADED(model1)) script::get_current()->yield(0ms);
	...
	void swordKnife()
	{
		...
		zkyo::g_CallbackScript->AddCallback<zkyo::ModelCallback>(MISC::GET_HASH_KEY("prop_cs_katana_01"), [=] {
			WEAPON::GIVE_DELAYED_WEAPON_TO_PED(PLAYER::PLAYER_PED_ID(), MISC::GET_HASH_KEY("WEAPON_KNIFE"), 1, TRUE);
			WEAPON::SET_CURRENT_PED_WEAPON(PLAYER::PLAYER_PED_ID(), MISC::GET_HASH_KEY("WEAPON_KNIFE"), 1);
			STREAMING::REQUEST_MODEL(MISC::GET_HASH_KEY("prop_cs_katana_01"));
			auto knife = WEAPON::GET_CURRENT_PED_WEAPON_ENTITY_INDEX(PLAYER::PLAYER_PED_ID());
			auto c = ENTITY::GET_ENTITY_COORDS(knife, 1);
			*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x9090;
			auto dildo = OBJECT::CREATE_OBJECT(MISC::GET_HASH_KEY("prop_cs_katana_01"), c.x, c.y, c.z, 1, 1, 0);
			*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x0574;
			ENTITY::SET_ENTITY_ALPHA(knife, 0, 0);
			Vector3 v0, v1;
			MISC::GET_MODEL_DIMENSIONS(MISC::GET_HASH_KEY("prop_cs_katana_01"), &v0, &v1);
	...
	/*bool features::money500k = false;
	void features::Money500k(bool toggle)
	{
		Notify("Recovery", "Money added", 7000, Success);
		if (toggle)
		{
			*script_global(1969112).as<int*>() = 1;

			*script_global(1969112).as<int*>() = 0;
		}
	}*/
	void features::de_remove_veh_crash()
	{
		...
		{
			Ped ped = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(features::g_selected_player);
