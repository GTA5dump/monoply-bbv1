...
namespace monopoly
{
	...
	bool hooks::received_event(rage::netEventMgr* event_manager, CNetGamePlayer* source_player, CNetGamePlayer* target_player, uint16_t event_id, int event_index, int event_handled_bitset, int unk, rage::datBitBuffer* buffer)
	{
				...
				switch (event_id)
				{
					case (int)RockstarEvent::NETWORK_CLEAR_PED_TASKS_EVENT:
					{
						if (features::no_freeze_event)
						{
							...
							g_fiber_pool->queue_job([=] { monopoly::Notify("Received Event", event_name, 4000, Protections); });
							g_pointers->m_send_event_ack(event_manager, source_player, target_player, event_index, event_handled_bitset);
							return false;
							}
					...
					case (int)RockstarEvent::NETWORK_INCREMENT_STAT_EVENT:
					{
						if (features::no_freeze_event)
						{
							if (!g_player_info.is_cutscene_playing() && !g_player_info.network_is_activity_session())
							{
								//4000
								//persist_modder::save(4000, 2, event_name);
								g_fiber_pool->queue_job([=] { monopoly::Notify("Received Scripted Game Event", event_name, 4000, Protections); });
								g_pointers->m_send_event_ack(event_manager, source_player, target_player, event_index, event_handled_bitset);
								return false;
							}
					...
					if (features::no_pickup_event)
					{
						...
						g_fiber_pool->queue_job([=] { monopoly::Notify(xorstr_("Received Event"), event_name, 4000, Protections); });
						break;
					}
					}
					case (int)RockstarEvent::NETWORK_CHECK_CODE_CRCS_EVENT:
					{
						...
						g_fiber_pool->queue_job([=] { monopoly::Notify((*name + "Is possible modder"+ *name), event_name, 4000, Protections); });
						break;
					}
					...
					case (int)RockstarEvent::REQUEST_CONTROL_EVENT:
					{
						...
						{
							g_fiber_pool->queue_job([=] { monopoly::Notify(xorstr_("Someone tried to freeze you"), event_name, 4000, Protections); });
		...
		void AssignNewPhysicalIndexHandler(CNetworkPlayerMgr* netPlayerMgr, CNetGamePlayer* player, uint8_t new_index) {
			const char* playerName = PLAYER::GET_PLAYER_NAME(player->m_player_id);

			if (new_index == static_cast<uint8_t>(-1))
			{
				char notification[128];
				sprintf(notification, ICON_FA_USER_MINUS"  %s has left the session.", playerName);
				// Notify about player leaving
				Notify(playerName, "Player Leaving", 7000, None);
			}
			else
			{
				char notification[128];
				sprintf(notification, ICON_FA_USER_PLUS"  %s has joined the session.", playerName);
				// Notify about player joining
				Notify(playerName, "Player Joining", 7000, None);
			}
		}
	...
	/*Crashes*/				g_fiber_pool->queue_job([=] { monopoly::Notify(xorstr_("Blocked crash"), sender->get_name(), 2000, Protections); });
							return true;
							}
						...
						case -1386010354: case 1757755807: case -393294520: case -371781708: case 1514515570:  case 911179316: case 846342319: case 2085853000: case -1970125962: case -1013679841: case -1767058336: case -1892343528: case 1494472464: case 296518236: case 998716537://put your hashes like that
							LOG(RAW_GREEN_TO_CONSOLE) << xorstr_("Blocked crash from: ") << sender->get_name();
	/*Kicks*/				g_fiber_pool->queue_job([=] { monopoly::Notify(xorstr_("Blocked crash"), sender->get_name(), 2000, Protections); });
							return true;
							}
